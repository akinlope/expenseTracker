...
const ExpenseDisplay = ({ dataFromBody }) => {
    ...
    return (
        <div className="  text-[#04eeff] pr-4 pl-4">
            {
                dataFromBody.map((elem) => {
                    return (
                        <div className=" flex justify-between border-2 mt-2 rounded-lg " key={elem.id}>
                            ...
                            {/* <DeleteExpense handleDelete={handleDelete} /> */}
                            {isDelete && <div onClick={()=>handleDelete(elem.id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-10 h-10 pt-2 cursor-pointer">
